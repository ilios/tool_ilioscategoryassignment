...
class ilios {
    ...
    public function __construct(
        ...
        $this->accesstoken = get_config('tool_ilioscategoryassignment', 'apikey') ?: '';
        $hosturl = get_config('tool_ilioscategoryassignment', 'host_url') ?: '';
    ...
    protected function parse_result(string $str): object {
        if (empty($str)) {
            throw new moodle_exception('erroremptyresponse', 'tool_ilioscategoryassignment');
        }
        $result = json_decode($str);

        if (empty($result)) {
            throw new moodle_exception('errordecodingresponse', 'tool_ilioscategoryassignment');
        }
    ...
    protected function validate_access_token(string $accesstoken): void {
        ...
        if ('' === trim($accesstoken)) {
            throw new moodle_exception('erroremptytoken', 'tool_ilioscategoryassignment');
        }
        ...
        if ($tokenpayload['exp'] < time()) {
            throw new moodle_exception('errortokenexpired', 'tool_ilioscategoryassignment');
        }
    }
    ...
    protected function get_access_token_payload(string $accesstoken): array {
        ...
        if (!$payload) {
            throw new moodle_exception('errordecodingtoken', 'tool_ilioscategoryassignment');
        }
        return $payload;
    }
}
